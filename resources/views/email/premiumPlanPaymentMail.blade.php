    ...
    <div class="wrap_body" style="background: #efefef;    display: inline-block;width: 100%;padding: 60px 0px;">
      <div class="wrap_container" style="background: #fff; margin: 0 auto; width: 80%;display: block;box-shadow: 1px 1px 15px -3px;">
        ...
        <div class="content-section" style="padding:45px 45px 10px 45px">
          <h4 style="color: #33c0c9;font-weight: bold;font-size: 22px;margin-bottom: 5px;">Hello {{ $userSubscriptionPlan->user->first_name.''.$userSubscriptionPlan->user->last_name }},</h4>
          <p style="padding-top: 10px;margin-bottom: 0px;"> Premium Plan Auto Renew Payment Details </p>
          @if($userSubscriptionPlan->is_auto_renew)
          <p style="line-height: 26px;text-align: justify;">Your {{ $userSubscriptionPlan->subscription->title }} plan subscription  is autorenew successfully of amount {{$userSubscriptionPlan->user_amount.' '.$userSubscriptionPlan->user_currency}}. </p>
          @else 
           <p style="line-height: 26px;text-align: justify;">We are Unable to receive your auto-renew payment for premium plan due to this reason: </p>
           <p>{{ $userSubscriptionPlan->transaction_message }}</p>
           <p> So, we are stop your auto-renew service on our site.</p>
           @endif
