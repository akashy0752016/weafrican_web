	...
	@include('includes.side-menu')
		<div class="col-md-9 col-sm-9">
			<div class="all_content">
				...
				<div class="panel-heading"> Edit Bank Account Details</div>
				<div class="panel-body">
            <form id="bankform" class="form-horizontal" action="{{ url('account-details/'.$account->id) }}" method="POST">
                {{csrf_field()}}
                 {{ method_field('PUT') }}
                <div class="form-group ">
                    <label for="category" class="col-md-3 col-sm-3 required control-label">Account Holder Name</label>
                    <div class="col-md-9 col-sm-9">
                        <input type="text" class="form-control" name="account_holder_name" value="{{ $account->account_holder_name}}" required>
                ...
                <div class="form-group ">
                    ...
                    <div class="col-md-9 col-sm-9">
                    	<input type="number" class="form-control" min=1 name="account_number" value="{{ $account->account_number}}" required>
                    ...
                    <div class="col-md-9 col-sm-9">
                    	<select required class="form-control" name="bank_name" required>
                            ...
                            @foreach($bankNames['data'] as $bankName)
                               <option value="{{ $bankName['name'] }}" @if($account->bank_name == $bankName['name']){{ 'selected'}} @else @endif  >{{ $bankName['name'] }}</option>
                            @endforeach
                        </select>
