...
{
    public $allowedExtentions = array();
    ...
    public function combineChunks($uploadDirectory)
    {
        ...
        return array("success" => true, "uuid" => $uuid, 'fileName' =>$name);
    }
    ...
    public function getMimeType($uuid, $fileName)
    {
        $finfo = finfo_open(FILEINFO_MIME_TYPE);
        $targetFilePath  =  config('image.media_temp_path').$uuid.'/'.$fileName;
        return finfo_file($finfo, $targetFilePath);
    }

    public function getFileType($mimType)
    {
        $mimTypeArray = explode('/', $mimType);
        if($mimTypeArray[0] == 'image') {
            return 'photo';
        }
        if($mimTypeArray[0] == 'audio') {
            return 'audio';
        }
        if($mimTypeArray[0] == 'video' || $mimType == 'application/octet-stream') {
            return 'video';
        }
        if($mimTypeArray[0] == 'text' || $mimTypeArray[0] == 'application') {
            return 'document';
        }
    }
}
