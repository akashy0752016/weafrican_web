    ...
    <div class="wrap_body" style="background: #efefef;    display: inline-block;width: 100%;padding: 60px 0px;">
      <div class="wrap_container" style="background: #fff; margin: 0 auto; width: 80%;display: block;box-shadow: 1px 1px 15px -3px;">
        ...
        <div class="content-section" style="padding:45px 45px 10px 45px">
          <h3><strong>Subject : </strong>New Business Registered on weafricans</h3>
          <h4 style="color: #33c0c9;font-weight: bold;font-size: 22px;margin-bottom: 5px;">Hello Admin,</h4>
          <p style="line-height: 26px;text-align: justify;">A new business register on weafricans.Here are the details:</p>
          <ul>
            <li>User Name - {{ $user->first_name.''.$user->middle_name.' '.$user->last_name}}</li>
            <li>User Email - {{$user->email}}</li>
            <li>User Mobile Number - {{$user->mobile_number}}</li>
            <li>Business Name - {{$user->business->title}}</li>
            <li>Business Category - {{$user->business->category->title}}</li>
            <li>Business Subcategory - @if(isset($user->business->bussiness_subcategory_id) && ($user->business->bussiness_subcategory_id != NULL && $user->business->bussiness_subcategory_id != "")) {{$user->business->subcategory->title}} @else -  @endif</li>
          </ul>
